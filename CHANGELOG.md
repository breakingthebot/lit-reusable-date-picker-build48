...
### Added
- Integrated **Date Picker Year & Month Quick Jump Picker** in `datePickerService.js` and `public/nexus-date-picker.js`.
- Added selectable year array generator (`getYearOptions`).
- Updated `<nexus-date-picker>` calendar header replacing static titles with interactive `<select class="month-select">` and `<select class="year-select">` controls.
- Added unit tests in `datePickerService.spec.js` (19 total unit tests passing).
